...
Object.values(keys).forEach((key) => (wooting_to_name[key.wooting] = key.name))
const razer_to_wooting = {}
Object.values(keys).forEach((key) => (razer_to_wooting[key.razer] = key.wooting))
const nuphy_to_wooting = {}
Object.values(keys).forEach((key) => (nuphy_to_wooting[key.nuphy ?? key.wooting] = key.wooting))
...
class AsProviderDrunkdeer extends AsProvider {
  ...
  startListening(handler) {
    ...
    this.interval = setInterval(function () {
      ...
      buf.set([
        0xb6, 0x03, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
...
class AsProviderKeychron extends AsProvider {
  ...
  startListening(handler) {
    ...
    buf.set([
      0xa9, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00
    ])
    ...
    if (
      this.dev.productId == 0x0b10 || // ANSI
  ...
  _requestAllKeys() {
    const buf = new Uint8Array(32)
    buf.set([
      0xa9, 0x31, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00
    ])
    this.dev.sendReport(0, buf)
...
class AsProviderMadlions extends AsProvider {
  ...
  startListening(handler) {
    if (
      this.dev.productId == 0x1055 ||
      this.dev.productId == 0x1056 ||
      this.dev.productId == 0x105d
    ) {
      this.layout = [
        KEY_ESCAPE,
        KEY_1,
    ...
    const buf = new Uint8Array(32)
    buf.set([
      0x02, 0x96, 0x1c, 0x00, 0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00
    ])
    this.dev.sendReport(0, buf)
...
const analogsense = {
  ...
  requestDevice: async function () {
    if ('hid' in navigator) {
      ...
      alert(
        'Your browser does not support the WebHID API, so analog keyboard input will not be available.'
      )
    }
    return null
  },
  ...
  drunkdeerIndexToHidScancode(i) {
    switch (i) {
      ...
      case 1 * 21 + 10:
        return 0x27 // KEY_0
      case 1 * 21 + 11:
        return 0x2d // KEY_MINUS
      case 1 * 21 + 12:
        return 0x2e // KEY_EQUALS
      case 1 * 21 + 13:
        return 0x2a // KEY_BACKSPACE
      case 1 * 21 + 15:
        return 0x4a // KEY_HOME
      ...
      case 2 * 21 + 10:
        return 0x13 // KEY_P
      case 2 * 21 + 11:
        return 0x2f // KEY_BRACKET_LEFT
      case 2 * 21 + 12:
        return 0x30 // KEY_BRACKET_RIGHT
      case 2 * 21 + 13:
        return 0x31 // KEY_BACKSLASH
      case 2 * 21 + 15:
        return 0x4b // KEY_PAGE_UP

      case 3 * 21 + 0:
        return 0x39 // KEY_CAPS_LOCK
      case 3 * 21 + 1:
        return 0x04 // KEY_A
      case 3 * 21 + 2:
        return 0x16 // KEY_S
      ...
      case 3 * 21 + 9:
        return 0x0f // KEY_L
      case 3 * 21 + 10:
        return 0x33 // KEY_SEMICOLON
      case 3 * 21 + 11:
        return 0x34 // KEY_QUOTE
      case 3 * 21 + 13:
        return 0x28 // KEY_ENTER
      case 3 * 21 + 15:
        return 0x4e // KEY_PAGE_DOWN

      case 4 * 21 + 0:
        return 0xe1 // KEY_LSHIFT
      case 4 * 21 + 2:
        return 0x1d // KEY_Z
      case 4 * 21 + 3:
        return 0x1b // KEY_X
      ...
      case 4 * 21 + 8:
        return 0x10 // KEY_M
      case 4 * 21 + 9:
        return 0x36 // KEY_COMMA
      case 4 * 21 + 10:
        return 0x37 // KEY_PERIOD
      case 4 * 21 + 11:
        return 0x38 // KEY_SLASH
      case 4 * 21 + 13:
        return 0xe5 // KEY_RSHIFT
      case 4 * 21 + 14:
        return 0x52 // KEY_ARROW_UP
      case 4 * 21 + 15:
        return 0x4d // KEY_END

      case 5 * 21 + 0:
        return 0xe0 // KEY_LCTRL
      case 5 * 21 + 1:
        return 0xe3 // KEY_LMETA
      case 5 * 21 + 2:
        return 0xe2 // KEY_LALT
      case 5 * 21 + 6:
        return 0x2c // KEY_SPACE
      case 5 * 21 + 10:
        return 0xe6 // KEY_RALT
      case 5 * 21 + 11:
        return 0x409 // KEY_FN (Custom mapping)
      case 5 * 21 + 12:
        return 0x65 // Key says "Menu" on it, doesn't seem to do anything.
      case 5 * 21 + 14:
        return 0x50 // KEY_ARROW_LEFT
      case 5 * 21 + 15:
        return 0x51 // KEY_ARROW_DOWN
      case 5 * 21 + 16:
        return 0x4f // KEY_ARROW_RIGHT
    }
